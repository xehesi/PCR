...
load_dotenv()
from mapping.SLAM import GraphSLAMTracker, draw_graph_map
import os, sys, cv2
import math
import numpy as np
...
def run_robot():
    ...
    tracker = GraphSLAMTracker(start_x=0.0, start_y=0.0, start_theta=0.0)
    ...
    print("Beginning maze exploration and mapping")

    left_speed = 0.0
    right_speed = 0.0
    bg_map = np.zeros((700, 700, 3), dtype=np.uint8)
    ...
    while robot.step(timestep) != -1:
        ...
        bg_map, live_map = draw_graph_map(
            tracker, 
            background_img=bg_map, 
            map_size=700, 
            scale=100, 
            center_mode="origin"
        )
        cv2.putText(live_map, f"Heading: {heading_deg:.1f} deg", (10, 25),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
        cv2.imshow("SLAM Graph Map", live_map)
